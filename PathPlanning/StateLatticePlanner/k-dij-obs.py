...
class DynamicPathVisualizer:
    def __init__(self, costmap, start, goal, planner, zoom_radius=75, info_received=False, frames_data=None, vehicle_history=None):
        self.costmap = costmap
        self.start = start
        self.goal = goal
        ...
        if info_received:
            ...
            self.vehicle_history = vehicle_history if vehicle_history is not None else []
        else:
            self.info_dist = [1355, 811]  # Points of interest for distance information
            self.frames_data = []
    ...
    def simulate_dynamic_planning(self, max_steps=2000):
        ...
        replan_frequency = max_steps  # Replan every max_steps
        ...
        for step in range(max_steps):
            ...
            distance_info = math.sqrt(
                (self.info_dist[0] - self.current_pos[0])**2 + 
                (self.info_dist[1] - self.current_pos[1])**2
            )

            if distance_info < 10:
                print(f"Distance to point of interest {self.info_dist}: {distance_info:.1f}")
                return True, self.frames_data, self.current_pos, self.vehicle_history
            ...
            if distance_to_goal < 5:
                print("Goal reached!")
                return False, self.frames_data, self.current_pos, self.vehicle_history
            ...
            if step % 2 == 0:  # Store every 2nd frame for smoother zoomed animation
                frame_data = {
                    ...
                    'vehicle_history': self.vehicle_history.copy(),
            ...
            if self.planner.current_optimized_segment and len(self.planner.current_optimized_segment) > 0:
                ...
                self.vehicle_history.append(list(self.current_pos))
...
def main():
    ...
    # Run simulation
    info, frames, pos, vehicle_history = visualizer.simulate_dynamic_planning(max_steps=1500)

    if info:
        start = [int(pos[0]), int(pos[1])]
        print(f"Updated start position for next run: {start}")
        grid_data = add_circular_obstacle(grid_data, center=[1599, 1089], radius=60)
        # plt.figure(figsize=(8, 8))
        # plt.imshow(grid_data, cmap='Blues', alpha=0.6, origin='upper')
        # plt.show()
        selected_path, alpha, beta = generate_k_alternative_paths(
            grid_data, start, goal, k=5, alpha=alpha, beta=beta)
        if selected_path is None:
        ...
        planner = DynamicHybridPlanner(grid_data, resolution=3, alpha=alpha, beta=beta)
        planner.global_dijkstra_path = selected_path
        visualizer = DynamicPathVisualizer(grid_data, start, goal, planner, zoom_radius=75, info_received=True,frames_data=frames, vehicle_history=vehicle_history)
