...
class SplitScreenVisualizer:
    def __init__(self, costmap, start, goal, planner, zoom_radius=75):
        ...
        self.fig, (self.ax_full, self.ax_zoom) = plt.subplots(1, 2, figsize=(24, 12))
        self.setup_plot()
    ...
    def setup_plot(self):
        """Setup the initial split-screen plot"""
        self.fig.suptitle('Dynamic Hybrid Path Planning', fontsize=20, fontweight='bold')
        
        # Full map view (left)
        self.ax_full.set_title('Full Costmap View', fontsize=16, fontweight='bold')
        self.ax_full.set_xlabel('Grid X (columns)', fontsize=12)
        self.ax_full.set_ylabel('Grid Y (rows)', fontsize=12)
        self.ax_full.grid(True, alpha=0.3)
        
        # Zoomed view (right)
        self.ax_zoom.set_title('Zoomed Robot View', fontsize=16, fontweight='bold')
        self.ax_zoom.set_xlabel('Grid X (columns)', fontsize=12)
        self.ax_zoom.set_ylabel('Grid Y (rows)', fontsize=12)
    ...
    def update_visualization(self, frame_num):
        """Update function for animation with split screen"""
        ...
        # Get zoom bounds around robot for the zoomed view
        x_min, x_max, y_min, y_max = self.get_zoom_bounds(current_pos)
        
        # ============ FULL MAP VIEW (LEFT SIDE) ============
        # Show full costmap
        self.ax_full.imshow(self.costmap, cmap='Blues', alpha=0.6, origin='upper')
        
        # Plot start and goal on full map
        self.ax_full.plot(self.start[1], self.start[0], 'go', markersize=10, 
                        label='Start', markeredgecolor='darkgreen', markeredgewidth=2)
        self.ax_full.plot(self.goal[1], self.goal[0], 'ro', markersize=10, 
                        label='Goal', markeredgecolor='darkred', markeredgewidth=2)
        
        # Plot global Dijkstra path on full map
        if frame_data['global_dijkstra_path'] and len(frame_data['global_dijkstra_path']) > 1:
            ...
            self.ax_full.plot(path_array[:, 1], path_array[:, 0], 'r--', 
                            linewidth=2, alpha=0.7, label='Global Dijkstra Path')
        
        # Plot vehicle trajectory history on full map
        if len(frame_data['vehicle_history']) > 1:
            history_array = np.array(frame_data['vehicle_history'])
            self.ax_full.plot(history_array[:, 1], history_array[:, 0], 'g-', 
                            linewidth=3, alpha=0.7, label='Vehicle Trajectory')
        
        # Highlight current robot position on full map
        self.ax_full.plot(current_pos[1], current_pos[0], 'o', 
                        color='orange', markersize=8, zorder=10,
                        markeredgecolor='black', markeredgewidth=2)
        
        # Draw rectangle showing the zoomed area
        rect = Rectangle((y_min, x_min), y_max-y_min, x_max-x_min, 
                        linewidth=2, edgecolor='black', facecolor='none', 
                        linestyle='--', label='Zoom Window')
        self.ax_full.add_patch(rect)
        
        # ============ ZOOMED VIEW (RIGHT SIDE) ============
        # Show zoomed costmap
        zoomed_costmap = self.costmap[x_min:x_max, y_min:y_max]
        self.ax_zoom.imshow(zoomed_costmap, cmap='Blues', alpha=0.6, origin='upper',
                        extent=[y_min, y_max, x_max, x_min])
        
        # Plot start and goal if they're in zoomed view
        if x_min <= self.start[0] <= x_max and y_min <= self.start[1] <= y_max:
            self.ax_zoom.plot(self.start[1], self.start[0], 'go', markersize=12, 
                            label='Start', markeredgecolor='darkgreen', markeredgewidth=2)
        
        if x_min <= self.goal[0] <= x_max and y_min <= self.goal[1] <= y_max:
            self.ax_zoom.plot(self.goal[1], self.goal[0], 'ro', markersize=12, 
                            label='Goal', markeredgecolor='darkred', markeredgewidth=2)
        
        # Plot global Dijkstra path in zoomed view
        if frame_data['global_dijkstra_path'] and len(frame_data['global_dijkstra_path']) > 1:
            path_in_view = []
            ...
            if len(path_in_view) > 1:
                ...
                self.ax_zoom.plot(path_array[:, 1], path_array[:, 0], 'r--', 
                                linewidth=2, alpha=0.5, label='Global Dijkstra Path')
        
        # Highlight current Dijkstra segment in zoomed view
        ...
        if len(frame_data['vehicle_history']) > 1:
            ...
            if len(history_in_view) > 1:
                ...
                self.ax_zoom.plot(history_array[:, 1], history_array[:, 0], 'g-', 
                                linewidth=3, alpha=0.7, label='Vehicle Trajectory')
        
        # Plot current robot position in zoomed view
        robot_circle = Circle((current_pos[1], current_pos[0]), 4, 
                            color='orange', alpha=0.9, zorder=10, 
                            edgecolor='black', linewidth=2)
        self.ax_zoom.add_patch(robot_circle)
        
        # Display all obstacles
        if 'obstacles' in frame_data and frame_data['obstacles']:
            for i, obstacle in enumerate(frame_data['obstacles']):
                obstacle_pos = obstacle['pos']
                obstacle_radius = obstacle['radius']
                just_added = obstacle.get('just_added', False)
                
                # Choose different colors for different obstacles
                obstacle_colors = ['red', 'darkred', 'firebrick', 'maroon', 'brown']
                color_idx = i % len(obstacle_colors)
                obstacle_color = obstacle_colors[color_idx]
                
                # Add obstacle to full view
                obstacle_circle_full = Circle((obstacle_pos[1], obstacle_pos[0]), obstacle_radius, 
                                            color=obstacle_color, alpha=0.5, zorder=9, 
                                            edgecolor='black', linewidth=2)
                self.ax_full.add_patch(obstacle_circle_full)
                
                # Add obstacle to zoomed view if in frame
                if x_min <= obstacle_pos[0] <= x_max and y_min <= obstacle_pos[1] <= y_max:
                    obstacle_circle_zoom = Circle((obstacle_pos[1], obstacle_pos[0]), obstacle_radius, 
                                            color=obstacle_color, alpha=0.5, zorder=9, 
                                            edgecolor='black', linewidth=2)
                    self.ax_zoom.add_patch(obstacle_circle_zoom)
                    
                    # Add label to zoomed view
                    self.ax_zoom.text(obstacle_pos[1], obstacle_pos[0] - obstacle_radius - 5, 
                                    f'OBSTACLE {i+1}', color=obstacle_color, fontsize=12, fontweight='bold',
                                    ha='center', va='bottom')
                
                # Add notification if obstacle was just added in this frame
                if just_added:
                    self.ax_full.text(0.5, 0.05 + 0.07*i, f"OBSTACLE {i+1} DETECTED!", 
                                    transform=self.ax_full.transAxes,
                                    fontsize=18, color=obstacle_color, fontweight='bold',
                                    ha='center', va='bottom',
                                    bbox=dict(facecolor='white', alpha=0.8, edgecolor=obstacle_color, boxstyle='round,pad=0.5'))
                    
                    if x_min <= obstacle_pos[0] <= x_max and y_min <= obstacle_pos[1] <= y_max:
                        self.ax_zoom.text(0.5, 0.05 + 0.07*i, f"OBSTACLE {i+1} DETECTED!", 
                                        transform=self.ax_zoom.transAxes,
                                        fontsize=18, color=obstacle_color, fontweight='bold',
                                        ha='center', va='bottom',
                                        bbox=dict(facecolor='white', alpha=0.8, edgecolor=obstacle_color, boxstyle='round,pad=0.5'))
        
        # Set zoom limits for zoomed view
        self.ax_zoom.set_xlim(y_min, y_max)
        self.ax_zoom.set_ylim(x_max, x_min)  # Inverted for image coordinates
        
        # Update legends
        self.ax_full.legend(loc='upper right', fontsize=10)
        self.ax_zoom.legend(loc='upper right', fontsize=10)
        
        # Update titles
        obstacle_count = len(frame_data.get('obstacles', []))
        obstacle_text = f" - {obstacle_count} Obstacle{'s' if obstacle_count != 1 else ''}" if obstacle_count > 0 else ""
        
        self.ax_full.set_title(f'Full Costmap View - Step {frame_data["step"]}{obstacle_text}', 
                            fontsize=16, fontweight='bold')
        self.ax_zoom.set_title(f'Zoomed View (Radius: {self.zoom_radius})\n'
    ...
    def simulate_dynamic_planning_with_multiple_obstacles(self, max_steps=2000, obstacle_configs=None):
        """
        Simulate dynamic planning with multiple obstacles appearing at different times
        
        Args:
            max_steps: Maximum simulation steps
            obstacle_configs: List of dictionaries with obstacle configurations
                Each dict should have: 
                - 'step': when obstacle appears
                - 'pos': [x,y] position (optional, if None will be placed on path)
                - 'radius': obstacle radius
        """
        print("Starting dynamic planning simulation with multiple obstacle perturbations...")
        
        # Initialize history and obstacle tracking
        vehicle_history = [list(self.current_pos)]
        obstacles_added = set()  # Track which obstacles have been added by their step
        
        # Use default obstacles if none provided
        if obstacle_configs is None:
            obstacle_configs = [
                {'step': 200, 'pos': None, 'radius': 25},
                {'step': 500, 'pos': None, 'radius': 35}
            ]
        ...
        for step in range(max_steps):
            ...
            # Check for any obstacles to add at this step
            for obs_config in obstacle_configs:
                obs_step = obs_config['step']
                
                # Add obstacle if it's time and hasn't been added yet
                if step == obs_step and obs_step not in obstacles_added:
                    obstacle_pos = obs_config.get('pos')
                    obstacle_radius = obs_config.get('radius', 20)
                    
                    # If position not provided, place on path ahead
                    if obstacle_pos is None and self.planner.global_dijkstra_path:
                        path_length = len(self.planner.global_dijkstra_path)
                        if self.current_path_index < path_length - 1:
                            # Place obstacle ~20% ahead on path
                            obstacle_idx = min(self.current_path_index + int(0.2 * path_length), path_length - 1)
                            obstacle_pos = self.planner.global_dijkstra_path[obstacle_idx]
                        else:
                            # Fallback: directly ahead of robot
                            dx = self.goal[0] - self.current_pos[0]
                            dy = self.goal[1] - self.current_pos[1]
                            dist = math.sqrt(dx*dx + dy*dy)
                            if dist > 0:
                                obstacle_pos = [
                                    int(self.current_pos[0] + dx * 0.3),
                                    int(self.current_pos[1] + dy * 0.3)
                                ]
                            else:
                                obstacle_pos = [
                                    int(self.current_pos[0] + 20),
                                    int(self.current_pos[1] + 20)
                                ]
                    
                    print(f"\n===== ADDING OBSTACLE #{len(obstacles_added)+1} AT STEP {step} =====")
                    print(f"Adding obstacle at position {obstacle_pos} with radius {obstacle_radius}")
                    
                    # Add circular obstacle to costmap
                    self._add_circular_obstacle(obstacle_pos, obstacle_radius)
                    
                    # Track this obstacle
                    obstacles_added.add(obs_step)
                    
                    # Store obstacle details for visualization
                    obs_config['pos'] = obstacle_pos  # Save actual position used
                    
                    # Force replanning with updated costmap
                    self.planner.global_dijkstra_path = None
            ...
            # Store frame data
            if step % 2 == 0:  # Store every 2nd frame for smoother animation
                # Find which obstacle was just added this frame (if any)
                just_added_obstacle = None
                for obs_config in obstacle_configs:
                    if obs_config['step'] == step and step in obstacles_added:
                        just_added_obstacle = obs_config
                
                # Create frame data with all obstacles
                frame_data = {
                    'step': step,
                    ...
                    'obstacle_added': len(obstacles_added) > 0,
                    'obstacles': [
                        {
                            'pos': obs['pos'],
                            'radius': obs['radius'],
                            'just_added': obs is just_added_obstacle
                        }
                        for obs in obstacle_configs if obs['step'] in obstacles_added and 'pos' in obs
                    ]
                }
    ...
    def simulate_dynamic_planning_with_obstacle(self, max_steps=2000, obstacle_step=300, obstacle_pos=None, obstacle_radius=15):
        """Simulate dynamic planning with an obstacle appearing during execution"""
        print("Starting dynamic planning simulation with obstacle perturbation...")
        
        vehicle_history = [list(self.current_pos)]
        
        # Flag for obstacle addition
        ...
        for step in range(max_steps):
            ...
            # Add obstacle at specified step
            if step == obstacle_step and not obstacle_added:
                # If obstacle_pos not provided, place on the path ahead
                if obstacle_pos is None and self.planner.global_dijkstra_path:
                    path_length = len(self.planner.global_dijkstra_path)
                    if self.current_path_index < path_length - 1:
                        # Place obstacle ~20% ahead on path
            ...
            if step % 2 == 0:  # Store every 2nd frame for smoother animation
                frame_data = {
                    ...
                    'obstacle_pos': obstacle_pos if obstacle_added else None,
                    'obstacle_radius': obstacle_radius if obstacle_added else None,
                    'obstacle_step': obstacle_step if step == obstacle_step and obstacle_added else None
                }
                self.frames_data.append(frame_data)
        ...
        for x in range(int(min_x), int(max_x) + 1):
            for y in range(int(min_y), int(max_y) + 1):
                ...
                if dist <= radius:
                    ...
                    self.costmap[x, y] = 1
                    self.planner.costmap[x, y] = 1
                    self.planner.state_lattice.costmap[x, y] = 1
        
        print(f"Added circular obstacle at ({center_x}, {center_y}) with radius {radius}")
    
    def create_animation(self, filename='split_screen_planning.gif', fps=10, format='gif'):
        """Create and save split-screen animation as GIF or MP4"""
        if len(self.frames_data) == 0:
            print("No frames to animate!")
            return None
            
        print(f"Creating split-screen animation with {len(self.frames_data)} frames...")
        ...
        if format.lower() == 'mp4':
            # For MP4 output, use ffmpeg writer
            filename = filename.replace('.gif', '.mp4') if filename.endswith('.gif') else filename
            
            print(f"Saving animation as MP4 to {filename}...")
            try:
                # Higher quality for MP4
                writer = animation.FFMpegWriter(
                    fps=fps, 
                    metadata=dict(artist='Dynamic Path Planner'),
                    bitrate=3600  # Higher bitrate for better quality
                )
                ...
                anim.save(gif_filename, writer='pillow', fps=fps, dpi=80)
...
def add_circular_obstacle(costmap,center, radius):
    """Add a circular obstacle with high cost to the costmap"""
    # Ensure center is valid
    center_x = max(0, min(center[0], costmap.shape[0] - 1))
    center_y = max(0, min(center[1], costmap.shape[1] - 1))
    
    # Calculate bounds of the affected region
    min_x = max(0, center_x - radius)
    max_x = min(costmap.shape[0] - 1, center_x + radius)
    min_y = max(0, center_y - radius)
    max_y = min(costmap.shape[1] - 1, center_y + radius)
...
def main():
    print("=== DYNAMIC HYBRID PATH PLANNING WITH MULTIPLE OBSTACLES ===")
    ...
    selected_path, alpha, beta = generate_k_alternative_paths(
        grid_data, start, goal, k=5, alpha=1.0, beta=1.0)
    ...
    # Create split-screen visualizer
    print(f"Creating split-screen visualization")
    visualizer = SplitScreenVisualizer(grid_data, start, goal, planner, zoom_radius=75)
    
    # Define multiple obstacles to appear at different times
    obstacle_configs = [
        {'step': 200, 'pos': None, 'radius': 40},  # First obstacle at step 200
        {'step': 500, 'pos': None, 'radius': 35}   # Second obstacle at step 500
    ]
    
    # Run simulation with multiple obstacles
    visualizer.simulate_dynamic_planning_with_multiple_obstacles(
        max_steps=2000,
    ...
    if len(visualizer.frames_data) > 0:
        animation_obj = visualizer.create_animation(
            filename=f'multiple_obstacles_planning.mp4', 
            fps=12,
            format='mp4'
        )
    ...
    print("Split-screen dynamic planning simulation completed!")
