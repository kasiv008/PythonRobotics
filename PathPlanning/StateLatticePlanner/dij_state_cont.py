...
class Dijkstra:
    def __init__(self, grid, start, goal):
        ...
        print(f"    DIJKSTRA INIT: Start {self.start} -> Goal {self.goal}")

    def find_path(self):
        nodes_explored = 0
        
        # REMOVED NODE LIMIT - Let Dijkstra explore completely
        while self.unvisited:
            ...
            # Debug output every 5000 nodes
            if nodes_explored % 5000 == 0:
                print(f"      Dijkstra explored {nodes_explored} nodes, current: ({x}, {y}), dist: {current_dist:.1f}")
            ...
            for nx, ny in self.get_neighbours(x, y):
                if not self.grid[nx, ny].visited:
                    ...
                    terrain_cost = max(1, float(self.grid[nx, ny].value))  # Use actual costmap values
        ...
        if not self.found_path:
            print(f"    DIJKSTRA FAILED: Explored {nodes_explored} nodes, no path found")
...
class DynamicHybridPlanner:
    def __init__(self, costmap, resolution=3):
        ...
        self.vehicle_speed = 3.0  # Increased for faster movement
    ...
    def plan_global_path(self, current_pos, final_goal):
        ...
        grid_distance = math.sqrt((final_goal[0] - current_pos[0])**2 + (final_goal[1] - current_pos[1])**2)
        real_distance = grid_distance * 3  # 3 meters per pixel
        print(f"  Grid distance: {grid_distance:.1f} units, Real distance: {real_distance:.0f} meters")
        ...
        try:
            ...
            if not dijkstra_path or len(dijkstra_path) < 2:
                ...
                print(f"  DIJKSTRA SUCCESS: {path_length} waypoints, {actual_distance:.0f}m path, {planning_time:.3f}s")
...
class DynamicPathVisualizer:
    ...
    def setup_plot(self):
        # Use better colormap to show cost variations
        self.ax.imshow(self.costmap, cmap='hot', alpha=0.7, origin='upper', vmin=0, vmax=100)
        self.ax.set_title('Dynamic Hybrid Path Planning - 2500x2500 @ 3m/pixel\nDijkstra Global + State Lattice Local', 
                         fontsize=16, fontweight='bold')
        self.ax.set_xlabel('Grid X (columns) - Each unit = 3 meters', fontsize=12)
        self.ax.set_ylabel('Grid Y (rows) - Each unit = 3 meters', fontsize=12)
    ...
    def update_visualization(self, frame_num):
        ...
        # Plot complete global Dijkstra path with better visibility
    ...
    def simulate_dynamic_planning(self, max_steps=150):  # Reduced for testing
        ...
        replan_frequency = 20  # More frequent replanning to see Dijkstra in action
        ...
        for step in range(max_steps):
            ...
            if distance_to_goal < 10:  # Within 30 meters
                print("Goal reached!")
                break
    ...
    def create_animation(self, filename='dijkstra_unlimited.gif', fps=8):
...
def main():
    print("=== UNLIMITED DIJKSTRA EXPLORATION ON LARGE COSTMAP ===")
    ...
    try:
        ...
        print(f"Real-world size: {grid_data.shape[0]*3}m x {grid_data.shape[1]*3}m")
    except FileNotFoundError:
        print("Costmap file not found!")
        return
    ...
    print("Dijkstra will now explore COMPLETELY without node limits!")
    ...
    if len(visualizer.frames_data) > 0:
        animation_obj = visualizer.create_animation(
            filename='dijkstra_unlimited_exploration.gif', 
            fps=10
        )
        plt.show()
    else:
