...
class SplitScreenVisualizer:
    ...
    def update_visualization(self, frame_num):
        ...
        # Display obstacle if it exists in the frame data
        if 'obstacle_added' in frame_data and frame_data['obstacle_added']:
            obstacle_pos = frame_data['obstacle_pos']
            obstacle_radius = frame_data['obstacle_radius']
            
            # Add obstacle to full view
            obstacle_circle_full = Circle((obstacle_pos[1], obstacle_pos[0]), obstacle_radius, 
                                        color='red', alpha=0.5, zorder=9, 
            ...
            if x_min <= obstacle_pos[0] <= x_max and y_min <= obstacle_pos[1] <= y_max:
                ...
                self.ax_zoom.text(obstacle_pos[1], obstacle_pos[0] - obstacle_radius - 5, 
                                'OBSTACLE', color='red', fontsize=12, fontweight='bold',
                                ha='center', va='bottom')
            
            # Add notification if obstacle was just added in this frame
            if frame_data.get('obstacle_step', -1) == frame_data.get('step', 0):
                self.ax_full.text(0.5, 0.05, "OBSTACLE DETECTED!", transform=self.ax_full.transAxes,
                                fontsize=18, color='red', fontweight='bold',
                                ha='center', va='bottom',
                                bbox=dict(facecolor='white', alpha=0.8, edgecolor='red', boxstyle='round,pad=0.5'))
                
                self.ax_zoom.text(0.5, 0.05, "OBSTACLE DETECTED!", transform=self.ax_zoom.transAxes,
                                fontsize=18, color='red', fontweight='bold',
                                ha='center', va='bottom',
                                bbox=dict(facecolor='white', alpha=0.8, edgecolor='red', boxstyle='round,pad=0.5'))
        ...
        # Update titles
        self.ax_full.set_title(f'Full Costmap View - Step {frame_data["step"]}', 
    ...
    def create_animation(self, filename='split_screen_planning.gif', fps=10):
        """Create and save split-screen animation"""
...
def main():
    print("=== DYNAMIC HYBRID PATH PLANNING WITH SPLIT-SCREEN VISUALIZATION ===")
    ...
    # Run simulation with dynamic obstacle
    obstacle_step = 300  # Add obstacle after 300 steps
    obstacle_radius = 60
    visualizer.simulate_dynamic_planning_with_obstacle(
        max_steps=1500,
    ...
    if len(visualizer.frames_data) > 0:
        animation_obj = visualizer.create_animation(
            filename=f'split_screen_planning_with_obstacle.gif', 
            fps=12
        )
